...
struct PLAYER_NAME : public Player {
   ...
   */
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; i++) {
				...
				if (pos_ok(pos) and !vis[pos.i][pos.j]) {
					...
					if(BalrogNearby(pos)) w = 2147483647;
					else if(cell(pos).type == Rock) w = dist[x][y]+cell(pos).turns+1;
	...
	bool BalrogNearby(Pos pos) {
		Pos init = unit(Balrog).pos + Dir(RT) + Dir(RT) + Dir(RT);
		Pos iterator = init;
		for(int i = 0; i < 7; i++) {
			for(int j = 0; j < 7; j++) {
				if(pos == iterator) return true;
				iterator = iterator + Dir(Right);
			}
			iterator = init + Dir(Bottom);
		}
		return false;
	}
