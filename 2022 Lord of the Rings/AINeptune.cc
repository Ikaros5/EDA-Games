...
struct PLAYER_NAME : public Player {
   ...
   */
	...
	PDI buscarEnemigo(int sourceI, int sourceJ) { 
		VVB vis(rows(), VB(cols(), false));
		VVI dist(rows(), VI(cols(), 2147483647));
		VD direcciones = {Top, Right, Bottom, Left, RT, BR, LB, TL};
		priority_queue <S, VS, greater<S>> q;
		int x, y, dis, w;
		Dir dir;
		bool first = true;
		...
		while (!q.empty()) {
			...
			for (int i = 0; i < 8; i++) {
				...
				if (pos_ok(pos) and !vis[pos.i][pos.j]) {
					vis[pos.i][pos.j] = true;
					if(first) dir = Dir(i);
					if (cell(pos).id != -1 and unit(cell(pos).id).player != me() and unit(cell(pos).id).type != Balrog and unit(cell(pos).id).type != Troll) return make_pair(dir, dis+1);
					if(cell(pos).type == Rock) w = dist[x][y]+cell(pos).turns+1;
					else if((cell(pos).type == Cave or cell(pos).type == Outside) and cell(pos).id == -1) w = dist[x][y]+1;
					else w = 2147483647;
		...
		return make_pair(None, dist[x][y]);
	}
