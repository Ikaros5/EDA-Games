...
//--------------------------------------------------------------------------------------------------------------------------------------------------//
	...
	// BFS to search for the closest path when in a city
	...
	bool mover_jugador(int x, int y, VVB& visited, queue <pair < pair<int, int>, pair<pair<Dir, int>, Unit>>> &q, Dir dir, int dis, Unit u) {
		...
		// BFS to search for the closest player when in a city
	pair<Dir, int> buscar_jugador_desde_ciudad(int i, int j) {
		queue <pair < pair<int, int>, pair<Dir, int>>> queue; // cola de posiciones, direcciones y distancias
		queue.push(make_pair(make_pair(i, j), (make_pair(NONE, 0)))); // insertamos en la cola la posicion, la direccion y la distancia iniciales
		VVB visited(rows(), VB(cols(), false));    // matrix de visitados de tamaño rows * cols
		visited[i][j] = true;
		...
		while (!queue.empty()) {
            ...
            if(rondas % 2 == 0) {
                //bottom
                if(first) dir = BOTTOM;
                if (mover_jugador_desde_ciudad(x+1, y, visited, queue, dir, dis)) return make_pair(dir, dis);
                ...
                //right
                if(first) dir = RIGHT;
                if (mover_jugador_desde_ciudad(x, y+1, visited, queue, dir, dis)) return make_pair(dir, dis);
	...
	bool mover_jugador_desde_ciudad(int x, int y, VVB& visited, queue <pair < pair<int, int>, pair<Dir, int>>> &q, Dir dir, int dis) {
		if (pos_ok(x, y) and !visited[x][y]) {
			if (cell(x, y).unit_id != -1 and is_enemy_unit(cell(x, y).unit_id)) return true;
			visited[x][y] = true;
			if (cell(x, y).type == CITY and cell(x, y).unit_id == -1) {
				q.push(make_pair(make_pair(x, y), make_pair(dir, dis+1)));
			}
	...
	// BFS to search for the shortest way to pos(di, dj)
	...
	bool is_enemy_unit(int id) {
		VI mis_unidades = my_units(me());
		int n = mis_unidades.size();
		for(int i = 0; i < n; i++) {
			if(id == mis_unidades[i]) return false;
		}
		return true;
	}
	/**
...
//------------------------------------------------------------------------------------------------------------------------------//
				if(dis_player <= 2) move(id, dir_player);
					...
					pair<Dir, int> move_player_from_city = buscar_jugador_desde_ciudad(u.pos.i, u.pos.j);
					Dir dir_player_from_city = move_player_from_city.first;
					int dis_player_from_city = move_player_from_city.second;
					if(dis_player_from_city < 10) {
						move(id, dir_player_from_city);
					}
					else {
